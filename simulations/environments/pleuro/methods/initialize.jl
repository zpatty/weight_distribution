function get_pleuro(; 
    ...
    α = 0.2
    bodies = [Cylinder(radius_b, height_b, mass_b, name=:body, color=Dojo.gray_light); [Cylinder(radius, height_1, mass, color=Dojo.gray_light) for i = 1:num_links]]
    ...
    if contact_body
        ...
        contacts1 = [
            ...
            [contact_constraint(bodies[i], normal,
                ...
                contact_radius=radius,
                contact_type=contact_type) for i=2:num_links+1];
            contact_constraint(bodies[end], normal,
                friction_coefficient=friction_coefficient,
                contact_origin=[0.0,0.0,height_1/2],
                contact_radius=radius,
                contact_type=contact_type) 
