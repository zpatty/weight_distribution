...
case = 5
case_name = ["distal"; "brittlestar"; "robot"; "central"; "long_dist"; "long_bstar"; "long_rob"; "long_central"]
case_mult = [1.8; 1.375; 1; 0.25; 1.8; 1.375; 1; 0.25]
# ## Patrick
...
for t = 1:25
    ū[t] = [zeros(4); -1.3; 1; zeros(env.num_inputs-10); -1.3; -1; zeros(2)]/2000
...
contact_location_storage = [copy([collect(i) for i in contact_location(env.mechanism)])]
#env.state .= get_minimal_state(env.mechanism)
...
for i=1:4
    for t = 1:12
        ...
        push!(contact_location_storage, copy([collect(i) for i in contact_location(env.mechanism)]))
    end
    for t = 13:25
...
max_coords = [minimal_to_maximal(env.mechanism, x) for x in y];
...
write(file, "state", y)
close(file)
file = matopen(path*"patrick_maximal_state_nominal_all_"*case_name[case]*".mat", "w")
write(file, "state", max_coords)
close(file)
file = matopen(path*"patrick_contact_location_nominal_"*case_name[case]*".mat", "w")
write(file, "contact_location", contact_location_storage)
close(file)
...
visualize(env, x̄);
xref = [[-1.0*t/25; 0.0; height; zeros(3); -0.5; zeros(5); x1[13:end]] for t=0:T-1]
# xref .= x_sol_old
timestep=0.05
...
qt_end = [100.0; 0.0; 100; 1.0*ones(3); 10.0; zeros(5); 100*ones(length(env.state)-12)]
...
@load joinpath(@__DIR__, "results/patrick_ilqr_distr_long_bstar-2023-05-10 15:24:48.jld2") x_sol u_sol
